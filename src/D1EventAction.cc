...
void D1EventAction::EndOfEventAction(const G4Event* event)
{
// Get hist collections IDs
 ...
 D1UserEventInformation* eventInformation
  ...
  D1ScintHitsCollection* scintHC = 0;
  D1PMTHitsCollection* pmtHC = 0;
  G4HCofThisEvent* hitsCE = anEvent->GetHCofThisEvent();
 
  //Get the hit collections
  if(hitsCE){
    if(fScintCollID>=0)scintHC = (D1ScintHitsCollection*)(hitsCE->GetHC(fScintCollID));
    if(fPMTCollID>=0)pmtHC = (D1PMTHitsCollection*)(hitsCE->GetHC(fPMTCollID));
  }
...
// Get sum values from hits collections
  //
  G4double sha1Edep = GetSum(GetHitsCollection(fSha1EdepHCID, event));
  G4double sha2Edep = GetSum(GetHitsCollection(fSha2EdepHCID, event));

  G4double sha1TrackLength 
    = GetSum(GetHitsCollection(fSha1TrackLengthHCID, event));
  G4double sha2TrackLength 
    = GetSum(GetHitsCollection(fSha2TrackLengthHCID, event));
...
// fill histograms
  ...
  analysisManager->FillH1(5, beta);
  analysisManager->FillH1(6, beta);
...
// fill ntuple
  ...
  analysisManager->FillNtupleDColumn(5, beta);
